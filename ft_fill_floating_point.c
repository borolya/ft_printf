												...
												t_floating_point *dst)
{
	...
	dst->sign = byte_repr[0] & 0x8000;
	dst->exponent = (byte_repr[0] & 0x7FFF) - 16383;
	dst->fraction = byte_repr[1] & 0x7FFFFFFFFFFFFFFF;
	dst->integer = (byte_repr[1] >> 63)& 0x1;
	dst->nan = dst->exponent == 16384 && dst->fraction != 0 ? 1 : 0;//add quite NAN
	dst->inf = dst->exponent == 16484 && dst->fraction == 0 ? 1 : 0;
    if (dst->exponent == -1023 && dst->fraction == 0)
	{
		dst->integer = 0;
		dst->exponent = 0;
